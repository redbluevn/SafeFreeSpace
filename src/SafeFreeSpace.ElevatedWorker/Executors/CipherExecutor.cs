...
public sealed class CipherExecutor : IOperationExecutor
{
    private readonly IProcessRunner _runner;
    private readonly IVolumeInventory _inventory;

    public CipherExecutor(IProcessRunner runner, IVolumeInventory inventory)
    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (current.MediaType != DriveMediaType.Hdd)
        {
            return ExecutorCommon.Fail(OperationErrorCategory.UnknownMediaType, "Media type is not HDD.");
        }
        ...
        BuiltCommand command = commandBuilder.BuildHddWipe(driveLetter);

        progress.Report(new OperationProgress(request.OperationId, "Running", $"Starting {Path.GetFileName(command.ExecutablePath)}"));
        ...
        {
            return ExecutorCommon.Fail(OperationErrorCategory.ToolNotFound, "cipher.exe not found.");
        }
